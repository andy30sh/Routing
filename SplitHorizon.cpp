            ...
            if (status == 0) {
                ...
                if (iss >> source >> dest >> cost) {
                    ...
                    if (cost > 0) {
                        ...
                        // Split Horizont amendment
                        printDistance();
                        round++;
                        nodes[source]->routes[getRouteKey(source, dest)] = make_tuple(dest, INFINITIVE);
                        nodes[dest]->routes[getRouteKey(dest, source)] = make_tuple(source, INFINITIVE);
                        for (const auto& node : nodes) {
                            if (node.first != source && node.first != dest) {                                
                                node.second->distances[getDistanceKey(node.first, source, dest)] = INFINITIVE;
                                node.second->distances[getDistanceKey(node.first, dest, source)] = INFINITIVE;
                            }
                        }
                        // End of split Horizont amendment
                    }
                }                
                isUpdate = true;
            }
...
void calculateDistance() {
    ...
    bool isChanged = true;
    string neighbor_by; // Split Horizont amendment
    int reversePoison = 0;  // Split Horizont amendment
                        ...
                        if (source.first != interface.first) {
                            ...
                            for (const auto& neighbor : router->neighbors) {
                                if (neighbor.first == interface.first) {
                                    ...
                                    neighbor_by = get<0>(neighbor.second->routes[getRouteKey(neighbor.second->name, dest.first)]);   
                                    if (!neighbor_by.empty() && neighbor_by != source.first) {  // Split Horizont amendment
                                        reversePoison = get<1>(nodes[neighbor_by]->routes[getRouteKey(neighbor_by, dest.first)]);                                        
                                    }                                    
                                    if (reversePoison == INFINITIVE) {  // Split Horizont amendment
                                        router->distances[getDistanceKey(source.first, dest.first, interface.first)] = INFINITIVE;  
                                        isChanged = true;
                                    } else if (neighbor_to_dest < INFINITIVE && source.first != neighbor_by) {                                        
                ...
                for (const auto& interface : nodes) {
                    if (source.first != interface.first) {
                        ...
                        if (distance < INFINITIVE)
                            ...
                            cout << left << setw(COLUMN_WIDTH) << LINK_DOWN; // Split Horizont amendment
                    }
                }
                cout << endl;
            }
